...
- 当前 UI 使用 `Green Red Light.zip` 导出的卡通欢迎页、倒计时和红灯场景素材。
...
- 机器人状态接口: `http://192.168.234.1:8876/robot/status`，由红绿灯项目自己的服务订阅 ROS `/highlevel_robotstate` 和 `/fault_info`，用于判断红灯前是否已经停稳。
...
2. 绿灯以动作为中心：每轮动作必须在完整 step/cycle 边界结束，不在半个 2 秒左转/右转中间切断。
3. 红灯以机器人状态为中心：Pad 先发 stop/必要恢复，再轮询 `/robot/status`，只有状态新鲜、无 fault、速度和角速度低于阈值时才显示红灯和播放 `Red Light！`。
4. 红灯显示后，机器人已经完成动作并停稳，不再在红灯期间执行普通动作恢复；特殊动作的恢复会在红灯显示前完成。
